...
//  pomumd
//
//  Centralized audio buffer conversion utility.
//  Consolidates audio conversion logic from TTSService and STTService.
//

import AVFoundation
import Foundation

/// Utility for converting between AVAudioPCMBuffer and Data formats
enum AudioBufferConverter {
    ...
    // MARK: - AVAudioPCMBuffer → Data Conversion

    /// Convert AVAudioPCMBuffer to Data
    /// Supports Int16, Int32, and Float32 formats
    /// Float32 is converted to Int16 for compatibility
    static func convertToData(_ buffer: AVAudioPCMBuffer) throws -> Data {
        ...
        var output = Data()
        output.reserveCapacity(frames * channels * 2) // Pre-allocate for Int16
        ...
        if let channelData = buffer.int32ChannelData {
            output.reserveCapacity(frames * channels * 4) // Adjust for Int32
            for frame in 0..<frames {
                for channel in 0..<channels {
                    var value = channelData[channel][frame].littleEndian
        ...
        // Handle Float32 format (convert to Int16)
    ...
    // MARK: - Data → AVAudioPCMBuffer Conversion

    /// Convert Data to AVAudioPCMBuffer
    /// Assumes Int16 PCM format
    static func convertToBuffer(from data: Data, format: AVAudioFormat) throws -> AVAudioPCMBuffer {
        guard data.count > 0 else {
            throw ConversionError.emptyBuffer
        }

        let frameCount = data.count / 2 // Assuming Int16 (2 bytes per sample)
    ...
    /// Detect AudioFormat from AVAudioPCMBuffer
